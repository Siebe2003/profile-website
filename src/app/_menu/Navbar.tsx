...
function Navbar() {
  ...
  return (
    <nav className="w-full fixed z-30 flex items-center text-lg px-4 h-nav navbar shadow-sm bg-background lg:px-10 2xl:px-14">
      ...
      <button
        className="cursor-pointer flex justify-end md:hidden text-foreground text-3xl"
        onClick={() => setIsCollapsed(!isCollapsed)}
      >
        {isCollapsed ? <GrMenu /> : <GrClose />}
      </button>
    </nav>
  );
}
